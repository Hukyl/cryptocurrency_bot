...
from utils._datetime import check_datetime_in_future, convert_from_country_format, convert_to_country_format
...
def get_currency_rates_today(msg, user):
    buttons_dct = {
    ...
    def choose_option_inner(msg):
        if buttons_dct.get(msg.text, None) is None:
            ...
            bot.send_message(
                msg.chat.id,
                _(
                    f"❗ Please aware, that time of alerts and forecasts is in UTC format only, which now is {convert_to_country_format(datetime.datetime.utcnow(), user.language)} ❗",
                    user.language
                )
            )
...
def convert_currency(msg):
    ...
    def get_isos(msg):
        nonlocal iso_from, iso_to
        try:
            iso_from, iso_to = msg.text.split('-')
        except ValueError:
    ...
    def print_convertation(msg):
        ...
        try:
            ...
            bot.send_message(
                msg.chat.id,
                _(
                    f"Конвертация на {convert_to_country_format(datetime.datetime.utcnow(), user.language)}:\
...
def change_user_rate_percent_delta(msg, user):
    currency = None
...
def change_user_rate_check_times(msg, user):
...
def subsription_payment_success(msg):
    ...
    datetime_expires = datetime.datetime.utcnow() + datetime.timedelta(days=n_months*31)
...
def send_techsupport_message(msg):
    ...
    if not user.is_staff:
        ...
        bot.send_message(
                msg.chat.id,
                _(
                    "❗ Please aware, that time of alerts and forecasts is in UTC format only ❗",
                    user.language
                )
            )
    else:
...
def check_premium_ended():
    def check_user_premium_ended(user):
        if datetime.datetime.utcnow() > user.is_pro:
            bot.send_message(
                user.user_id,
...
def check_alarm_times():
    previous_minute = 0
    while True:
        t_ = datetime.datetime.utcnow().time()
